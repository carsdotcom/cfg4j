...
/**
 * Decorator for {@link SimpleConfigurationProvider} that emits execution metrics. It emits the
 * following metrics (each of those prefixed with a string passed at construction time):
 * <ul>
...
class MeteredConfigurationProvider implements ConfigurationProvider {
  ...
  /**
   * Create decorator for given {@code delegate} and using {@code metricRegistry} for constructing
   * metrics. Each metric will be prefixed with {@code metricPrefix}
   *
   ...
   */
  MeteredConfigurationProvider(MetricRegistry metricRegistry, String metricPrefix,
      SimpleConfigurationProvider delegate) {
    requireNonNull(metricRegistry);
    requireNonNull(metricPrefix);
    this.delegate = requireNonNull(delegate);

    allConfigurationAsPropertiesTimer =
        metricRegistry.timer(metricPrefix + "allConfigurationAsProperties");
